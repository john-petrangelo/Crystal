...
err_t LogServer::onAccept(void *arg, tcp_pcb *newpcb, [[maybe_unused]] err_t err) {
  ...
  logger.set(new NetworkLogger(newpcb));
  logger << "Switched to network logger" << std::endl;

  return ERR_OK;
}

err_t LogServer::onReceive(void *arg, tcp_pcb *tpcb, pbuf *p, err_t err) {
  auto server = static_cast<LogServer*>(arg);
  if (err == ERR_OK) {
    if (p != nullptr) {
...
void LogServer::onError(void *arg, err_t err) {
  ...
  server->closeConnection(static_cast<tcp_pcb*>(arg)); // Handle error
}

void LogServer::closeConnection(tcp_pcb *tpcb) {
...
void LogServer::sendMessage(tcp_pcb *tpcb, char const *msg, size_t const msgLen) {
  err_t err;
