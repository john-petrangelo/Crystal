...
static void dns_server_process(void *arg, struct udp_pcb *upcb, struct pbuf *p, const ip_addr_t *src_addr, u16_t src_port) {
  ...
  // JHP TODO REMOVE next 2 lines
//  uint16_t flags = lwip_ntohs(dns_hdr->flags);
//  uint16_t question_count = lwip_ntohs(dns_hdr->question_count);
  flags = lwip_ntohs(dns_hdr->flags);
  question_count = lwip_ntohs(dns_hdr->question_count);
...
//  const uint8_t *question_ptr = question_ptr_start;
  question_ptr_start = dns_msg + sizeof(dns_header_t);
  question_ptr_end = dns_msg + msg_len;
  ...
  // Generate answer
// JHP TODO REMOVE uint8_t *answer_ptr = dns_msg + (question_ptr - dns_msg);
  answer_ptr = dns_msg + (question_ptr - dns_msg);
